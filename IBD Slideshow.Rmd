...
style_duo_accent(
  ...
  title_slide_background_color=   "#ec9120",
...
background-image: url(IBD-Slideshow_files/figure-html/harvard-bil.png)
...
1. Excellent <font color="#006cc5">**learning opportunity**</font color>

--

1. Provides another chance to <font color="#006cc5">**validate SAS results**</font color> prior to publication

--

1. R provides a wider variety of add-on packages that can enhance preliminary output - <font color="#006cc5">**makes sharing with collaborators fun**</font color>

--

1. Data scientists have made a marked <font color="#006cc5">**shift to R and Python**</font color> and away from SAS and SPSS
...
background-image: url(IBD-Slideshow_files/figure-html/keyboard2crop2-unsplash.jpeg)
background-size: cover
...
Here's area few code snippets from the  <font color="#006cc5">**dplyr**</font color> and <font color="#006cc5">**broom**</font color> packages that we used to wrangle our data:
...
<center><bold>Note the use of the pipe <font color="#006cc5"><b>( %>% )</b></font color>  operator</bold></center> #<<
...
<img src="IBD-Slideshow_files/figure-html/skinny-arrow.png" height="50px",  class="arrow"  />
...
.pull-right[
<center><img src="IBD-Slideshow_files/figure-html/kable-summary6.png" height="400px" width=750px" style="padding-top:60px"/></center>  ]
...
trialcount<-   sqldf(" select count(distinct(ID)) from SITES ")  %>% pull()
  ...
  SITES2 <-
    ...
    summarise( COUNT = n()) %>% 
    arrange( desc(COUNT) ) %>% 
    mutate( PERCENT =  round( (COUNT / trialcount ), 3)  ) %>% 
  ...
  kbl(SITES2, booktabs = T, format= "html", 
      ...
      kable_paper(full_width = F, html_font= "Helvetica Neue", "hover", font_size=16) %>% column_spec(1, width = "10cm") %>% 
      column_spec(2:5, "3cm") %>% 
...
ggplot(data = WORLDMAP, aes( fill= QRTL)) +
 ...
 # annotation_scale(location = "br", width_hint = 0.15, plot_unit = "km" ) +
  ...
  # coord_sf( crs= " +proj=merc", expand= F)
...
background-image: url(IBD-Slideshow_files/figure-html/blkasian-unsplash.jpg)
background-size: cover
...
<center><img src="IBD-Slideshow_files/figure-html/race-prop2.png" width="700px" /></center>
<!-- <img src="IBD-Slideshow_files/figure-html/big-red-x.png" height="100px",  class="smallx"  /> -->
<small><br></small>
...
```{r Race-Collection, eval=F, echo=F}
require(treemap)
...
ggplot( data= SCIMP, aes(x=RACE, y=PROPORTION, color=RACE, group= 1)) +
  geom_bar( stat='identity', alpha=0.75, position = "dodge", aes(fill= RACE ), fill='darkgrey', color='black', lwd=0.25, ymin=0, ymax=1) +
...
---
## Sample of IBD Prevalence Rates from another Study

```{r KPNC-Prevalence, echo=F, out.width= "85%", fig.align="center"}

KPNC <- data.frame(  RACE= c('ASN','BLK','HIS','WHT','OTH'),
                       PREV= c( 131, 271, 182, 388, 128),
                       CUSTLABEL = c('Asian','Black','Hispanic', 'White', 'Other') )

KPNC$RACE <- factor( KPNC$RACE, levels= c('ASN','BLK','HIS','WHT', 'OTH') )


ggplot( KPNC, aes( x= RACE , y=PREV )) +
  geom_col(     aes( x= factor(RACE, level= c('OTH','WHT','HIS','BLK','ASN')) , y=PREV ), width=0.25, color="#006cc5", fill="green", alpha=0.5 ) +
  geom_segment( aes(  x=RACE, xend= RACE, y=0, yend=PREV), linetype = 'dotted', lwd=0.5, color="#006cc5", alpha=0.5)+
  scale_x_discrete( labels= c('Other', 'White', 'Hispanic', 'Black', 'Asian') ) +
  geom_point( aes(x=RACE), size=12, color="#006cc5" )+
  geom_text(  aes( x= RACE , label=PREV ),hjust=0.5, vjust=0.5, color="white", fontface="bold", size=4 ) +
  coord_flip() +
  theme_bw()+
...
- In their abstract, <b>[DeLisser et al. (2018)](https://journals.lww.com/ajg/Fulltext/2018/10001/Variation_in_IBD_Prevalence_by_Race_Ethnicity.607.aspx)</b> used claims data from CY14 from  8,064 members of the Kaiser-Permanente Northern California health plan to compute Prevalence Rates.

- Their findings showed statistically significant higher rates among Whites relative to all other races (depicted above, <i>p= <0.001</i>), as well as among women and the elderly.
...
census2 <- c( 0.508, 0.492)
  ...
  IBD2 %>% 
    ...
    geom_bar( stat='identity',  position = "dodge", aes(fill= METRIC, color=METRIC ), color="black", lwd=0.25, alpha=0.75, width=0.575) +
...
---
class: inverse, hide-count
background-image: url(IBD-Slideshow_files/figure-html/grandmotherblue-unsplash.jpg)
background-size: cover


## Task No. 4
### Report the <font color="#ef8de4">age demographics</font color> obtained from those IBD trials where such information was collected.
---
## Inclusion of Children & Seniors in IBD Trials
```{r, Child-Participation, echo=F,eval=F}
...
```{r, Child-Participation-Revised, echo=F,eval=F}

require(plotrix)

# child plot2 

csurvey <- c(NO=.8220, YES=.1780 )
cpielabels <- paste(names(csurvey),"\n",percent(substr(round(csurvey,3),1,5)))

childpie2<-
  pie3D(csurvey,
        col=c("darkgrey", "#006cc5"),
        start = .35,
        labelcex = 2,
        radius = 1.5,
        explode=0.2,
...
#  ggsave(path= "IBD-Slideshow_files/figure-html", filename= "child-pie2.png", plot= last_plot(), width = 5, height = 5, dpi=300)
...
.pull-left[
<center><b>Percentage of Trials with Child Enrollees</b></center>
<img src="IBD-Slideshow_files/figure-html/child-pie2.png" height="400px" />
<img src="IBD-Slideshow_files/figure-html/childbw-emoji.png" height="75px",  class="childemoji"  />
]

```{r Senior-Participation, echo=F, eval=F}
...
```{r Senior-Participation-Revised, echo=F, eval=F}
...
ssurvey <- c(NO=.6440, YES=.3560 )
spielabels <- paste(names(ssurvey),"\n",percent(substr(round(ssurvey,3),1,5)))
...
#  ggsave(path= "IBD-Slideshow_files/figure-html", filename= "senior-pie2.png", plot=last_plot(), width = 5, height = 5, dpi=300)


```

.pull-right[
<center><b>Percentage of Trials with Senior Enrollees</b></center>
<img src="IBD-Slideshow_files/figure-html/senior-pie2.png" height="400px" />
<img src="IBD-Slideshow_files/figure-html/senior2bw-emoji.png" height="75px",  class="senioremoji"  />
]
<center>Children were included in 26 studies, while adults over the age of 65 were enrolled in 52 studies</center>

---
## Inclusion of Children & Seniors by Type of Trial
<img src="IBD-Slideshow_files/figure-html/red-emphasis.png" height="200px",  class="oval1"  />
```{r Trial-Type-Child-Seniors, echo=F}
  require(sjPlot)
    ...
    plot_xtab(X$CAT2, X$INCL, margin = "row", bar.pos = "stack", coord.flip = TRUE,
             ...
             # geom.colors = c("#006cc5","#a6e17f", "#cbcccb"),
             axis.labels= c("CD","UD","Other"),
             axis.titles = c('', ''), legend.title= "Included?",wrap.labels=5,  legend.labels = c("Children", "Neither", "Seniors"),
            show.summary = TRUE )+theme_classic()
```
<center>Children and Seniors were largely excluded from trials in the two main IBD trials categories:<br>Crohn's Disease (CD) & Ulcerative Colitis (UC)</center>
...
$$Y_{i} = \beta_{0} + \beta_{1}{X}_{1}  +  \beta_{2}{X}_{2}  +  . . . . \beta_{n}{X}_{n} +  \epsilon_{i}$$

---
## Two images & a superimposition
<img src="IBD-Slideshow_files/figure-html/big-red-x.png" height="500px",  class="bigx"  />
.pull-left[
<center><img src="IBD-Slideshow_files/figure-html/loren-oval.jpg" height="480px", width= "380px" /></center>
]

.pull-right[
<!-- <img src="IBD-Slideshow_files/figure-html/kevin-oval.jpg" height="500px", width= "400px" /> -->
<img src="IBD-Slideshow_files/figure-html/blurry-kevin.jpg" height="480px", width= "380px" />
]
